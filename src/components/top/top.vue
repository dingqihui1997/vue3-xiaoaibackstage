        ...
        >
          ...
          >
            ...
            >
              <i
                class="el-icon-close"
                v-if="item.name !== '首页' || (list.length !== 1 && !flag)"
              ></i>
            </div>
          </div>
          <div
...
export default defineComponent({
  setup() {
    ...
    let other = (item: any) => {
      show.value = false;
      if (list.value.length > 1) {
        arr.value = list.value.filter((item1: any) => {
          return item.name === item1.name;
        });
        store.commit("settabList", JSON.stringify(arr.value));
        localStorage.setItem("tabList", JSON.stringify(arr.value));
        router.push(item.url);
      } else {
        ElMessage.warning("已经是最后一项啦");
      }
    };
    // 关闭左侧
