...
export default defineComponent({
  setup() {
    ...
    let click = (name: string, url: string) => {
      console.log(list);
      let navobj = ref<Obj2>({ name: name, url: url });
      console.log(navobj.value);
      let a = list.find((item: any) => {
        return item.name === name;
      });
      if (!a) {
        list.push(navobj.value);
      }
      store.commit("settabList", JSON.stringify(list));
      localStorage.setItem("tabList", JSON.stringify(list));
