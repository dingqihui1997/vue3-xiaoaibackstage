      ...
      <div>
        <el-tabs tab-position="left" @tab-click="click" v-model="activeName">
          ...
          <el-tab-pane label="商品内容" name="4" :disabled="!(active >= 4)"
            ><div id="div1"></div>
            <el-button class="butt" type="success" @click="next(5)"
              >提交</el-button
            >
          </el-tab-pane>
        </el-tabs>
...
export default defineComponent({
  setup() {
    ...
    let fourth = ref<boolean>(true); //上传图片
    let last = ref<boolean>(true); //商品内容
    // let num =ref<number></number>
   ...
   let   attrs =only.value.concat(many.value)
      api.addgoods({
        goods_name:edit.value.goods_name,goods_cat:edit.value.goods_cat.join(),
        goods_price:Number(edit.value.goods_price),goods_number:Number(edit.value.goods_number),
        goods_weight:Number(edit.value.goods_weight),goods_introduce:content.value,
        pics:img.value,attrs:
      })
    };
    // 获取商品参数静态
    let categories = () => {
