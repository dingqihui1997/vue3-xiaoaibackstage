...
export default defineComponent({
  setup() {
    ...
    let deletecategories = () => {
      api
        .deletecategories({ id: optionsvalue.value[2], attrid: attrId.value })
        .then((res: any) => {
          if (res.meta.status === 200) {
            ElMessage.success(res.meta.msg);
            categories();
    ...
    let close = (row: any, item: any) => {
      console.log(item);
      console.log(row);
      attr_vals.value = item;
      attrId.value = row.attr_id;
      id.value = row.cat_id;
      attr_name0.value = row.attr_name;
    };
    let updateAttributesById = () => {
      // api.updateAttributesById({id:, attrId, attr_name, attr_sel, attr_vals})
    };
    onMounted(() => {
      getcategories();
    });
