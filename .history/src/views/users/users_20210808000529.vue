...
export default defineComponent({
  ...
  setup() {
    ...
    let change = (item: any) => {
      id.value = item.id;
      state.value = item.mg_state;
      api
        .stateUsers({ uId: id.value, type: state.value })
        .then((res: any) => {
    ...
    let distribution = () => {
      ...
      api
        .distributionUsers({ id: id.value, rid: Number(rid.value) })
        .then((res: any) => {
          flag.value = false;
          success(res);
        })
        .catch((err) => {
          console.log(err, "分配角色失败");
        });
    };
    // 添加用户
    let getAddusers = () => {
