                  ...
                  <div class="flex-a boxtag">
                    <div class="tag">
                      <el-tag
                        ...
                        @close="close(props.row.id, item.id)"
                      >
                        {{ item.authName }}
                      </el-tag>
                      ...
                      >
                        <div class="twotag">
                          <el-tag
                            ...
                            :disable-transitions="false"
                            type="success"
                            @close="close(props.row.id, item1.id)"
                          >
                            {{ item1.authName }}
                          </el-tag>
                          <i class="el-icon-caret-right"></i>
                        ...
                        <div class="flex">
                          ...
                          >
                            <el-tag
                              type="warning"
                              closable
                              @close="close(props.row.id, item2.id)"
                            >
        ...
        <el-table-column label="操作" prop="desc" align="center">
          <template #default="scope">
            ...
            <el-button type="danger" size="mini" @click="handle(scope.row, 2)"
              >删除</el-button
            >
            <el-button type="warning" size="mini" @click="handle(scope.row, 3)"
              >分配权限</el-button
            >
          </template>
        </el-table-column>
...
export default defineComponent({
  setup() {
    ...
    const handle = (item: any, num: number) => {
      ...
      if (num === 1) {
        ...
        item.children.map((item1: any) => {
          idlist.value.push(item1.id);
          item1.children.map((item2: any) => {
            idlist.value.push(item2.id);
        ...
        if (tree.value) {
          ...
          tree.value.setCheckedNodes(item.children);
        }
      }
    };
    // 添加角色按钮
    ...
    const close = (roleId: number, rightId: number) => {
      api
        .deloneroles({ roleId: roleId, rightId: rightId })
        .then((res) => {
          console.log(res);
          success(res);
        })
    ...
    const determine = () => {
      let a = ref<any>([]);
      a.value = tree.value.getCheckedKeys();
      console.log(a.value);
      a.value.map((item: any) => {
        str.value += String(item) + ",";
      });
    ...
    const roleId = () => {
      api.roleId({roleId:id.value,rids:str.value}).then(res=>{
        console.log(res)
      }).catch(err=>{
        console.log(eer,'获取角色失败')
      })
    };
