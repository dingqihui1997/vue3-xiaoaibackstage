...
export default {
  ...
  // 添加用户
  getAddusers({ username, password, email, mobile }: { username: string, password: string, email: string, mobile: string }) {//添加用户
    return http.post('/users', {
      username, password, email, mobile
    })
  },
  //编辑用户提交
  getEditusers({ id, email, mobile }: { id: number, email: string, mobile: string }) {
